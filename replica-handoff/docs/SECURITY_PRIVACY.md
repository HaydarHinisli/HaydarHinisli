...
**Origin allowlist (Fix-Sprint, ADR-051, ordering hardened by ADR-052)**: the JWT
check above proves WHO is connecting, not WHERE the connecting page is served
from. `/ws/live/{call_id}` additionally checks the `Origin` header against
`REPLICA_ALLOWED_WS_ORIGINS` (`app/services/ws_origin.py`) — BEFORE
`websocket.accept()`, so a disallowed origin is rejected at the handshake and
never receives an accepted WebSocket at all, rather than being accepted and
immediately dropped. Fails closed outside `REPLICA_ENV=local`: an empty/unset
