...
Role assertions use the DEFAULT resolver's mapping, corrected by ADR-053 for the
confirmed first-real-test topology: INBOUND = seller (caller-in), OUTBOUND =
prospect (the `<Dial>`-ed-out party)."""
...
def test_speaker_change_detected_between_alternating_tracks():
    ...
    assert td.last_active_speaker == 'prospect'
...
def test_turn_end_resolves_to_exactly_one_turn_event_with_correct_speaker():
    ...
    td.on_vad_update(INBOUND, True, now_monotonic=0.0)
    td.on_vad_update(INBOUND, False, now_monotonic=1.0)
    turn = td.on_turn_ended(INBOUND, 'Guten Tag hier ist Anna von Replica.', now_monotonic=1.0)
    assert turn is not None
    assert turn.speaker == 'seller'
    assert turn.text == 'Guten Tag hier ist Anna von Replica.'
...
def test_turn_end_flags_had_overlap_when_overlap_occurred_during_the_utterance():
    ...
    turn_prospect = td.on_turn_ended(OUTBOUND, 'kurzer Einwurf', now_monotonic=0.8)
    assert turn_prospect.had_overlap is True

    td.on_vad_update(INBOUND, False, now_monotonic=1.2)
    turn_seller = td.on_turn_ended(INBOUND, 'langer Satz', now_monotonic=1.2)
    assert turn_seller.had_overlap is True  # both sides flagged, neither adjudicated as "winner"
...
def test_each_track_finalizes_independently_even_when_overlapping():
    ...
    prospect_turn = td.on_turn_ended(OUTBOUND, 'prospect text', now_monotonic=0.6)
    td.on_vad_update(INBOUND, False, now_monotonic=1.0)
    seller_turn = td.on_turn_ended(INBOUND, 'seller text', now_monotonic=1.0)
